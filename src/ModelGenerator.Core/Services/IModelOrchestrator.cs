...
/// <summary>Orchestrates the full shape -> text/svg/image -> composed mesh -> STL workflow.</summary>
public interface IModelOrchestrator
{
    ...
    /// each text/SVG/image item's mesh, separate instead of merged into one mesh — lets the UI
    ...
    (Mesh Floor, Mesh Border, IReadOnlyList<PositionedTextMesh> TextMeshes, IReadOnlyList<PositionedSvgMesh> SvgMeshes, IReadOnlyList<PositionedImageMesh> ImageMeshes) GenerateModelParts(Model model);
