...
/// <summary>Holds one ImageInsertEditorControl row per inserted photo, plus an "Insert Image..."
/// button that opens the library dialog — mirrors SvgInsertsPanel.</summary>
public class ImageInsertsPanel : UserControl
{
    ...
    private readonly Button _addInsertButton = new() { Text = "+ Insert Image...", AutoSize = true, Dock = DockStyle.Top };

    public event EventHandler? InsertsChanged;

    public ImageInsertsPanel(IImageLibraryService imageLibrary)
    {
    ...
    public List<ImageInsert> Inserts =>
        _rowsPanel.Controls.OfType<ImageInsertEditorControl>().Select(c => c.ToImageInsert()).ToList();

    /// <summary>Replaces all rows with the given inserts, in LineNumber order.</summary>
    public void LoadInserts(IEnumerable<ImageInsert> inserts)
    {
        Clear();
        foreach (var insert in inserts.OrderBy(i => i.LineNumber))
        {
            var row = CreateRow();
            row.LoadFrom(_imageLibrary, insert);
    ...
    {
        var row = _rowsPanel.Controls.OfType<ImageInsertEditorControl>().FirstOrDefault(r => r.LineNumber == insertIndex);
        row?.SetManualPosition(x, y, z);
    }

    public void Clear()
    {
        foreach (var row in _rowsPanel.Controls.OfType<ImageInsertEditorControl>().ToList())
        {
    ...
    {
        using var dialog = new ImageLibraryDialog(_imageLibrary);
        if (dialog.ShowDialog(FindForm()) != DialogResult.OK || dialog.SelectedImageData is null)
        {
            return;
        }

        var row = CreateRow();
        row.SetImageSource(_imageLibrary, dialog.SelectedFileName, dialog.SelectedImageData);
        _rowsPanel.Controls.Add(row);
        LayoutRows();
        InsertsChanged?.Invoke(this, EventArgs.Empty);
    }

    private ImageInsertEditorControl CreateRow()
    {
        var row = new ImageInsertEditorControl
        {
    ...
    private void RemoveInsert(ImageInsertEditorControl row)
    {
        _rowsPanel.Controls.Remove(row);
        row.Dispose();
        RenumberInserts();
    ...
    {
        int i = 0;
        foreach (var row in _rowsPanel.Controls.OfType<ImageInsertEditorControl>())
        {
            row.LineNumber = i++;
        }
    }
